...
Springer Natureの場合、[SharedIt](https://www.springernature.com/jp/researchers/sharedit)というサービスがある。このサービスはダウンロードできないものの、ブラウザ上で論文の内容を雑誌に掲載されている形式を無料で読むことができる。この出版社で論文が受理されたときは、必ずリンクを取得し、積極的に広報しよう。


### オープンアクセス以外
どの方法を調べても、論文を入手できない場合がある。最終手段として、その論文の責任著者に直接メールを送り、論文をもらう方法がある。自分の論文に興味があると連絡が来て、気分が悪くなる研究者はいないはずなので、とりあえず連絡してみるとよい。
